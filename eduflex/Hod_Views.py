...
from eduflexApp.models import Attendance,Attendance_Report,Course,Session_Year,CustomUser,Student,Staff,Subject,Staff_Notification,Staff_leave,Staff_Feedback,Student_Notification,Student_Feedback,Student_leave
...
def UPDATE_STAFF(request):
    ...
    if request.method == "POST":
        ...
        profile__pic = request.FILES.get('profile_pic')
        first_name = request.POST.get('first_name')
        last_name = request.POST.get('last_name')
        email = request.POST.get('email')
        username = request.POST.get('username')
        ...
        if password!=None and password!="":
            user.set_password(password)
        if profile__pic!=None and profile__pic!="":
            user.profile_pic = profile__pic
...
def DELETE_STAFF(request,admin):
    ...
    staff.delete()
    messages.success(request,'Record are Sucessfully Deleted !')
    return redirect('view_staff')
...
def ADD_SESSION(request):
    ...
    if request.method == 'POST':
        ...
        session.save()
        messages.success(request,'Sesssion Are Successfully Created! ')
